...
class LstmRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=1, output_size=1, num_layers=1):
        ...
        # 全连接层，张量从hiddensize的1变为output的1
        self.forwardCalculation = nn.Linear(hidden_size, output_size)
        # 隐藏层
        # self.hidden = (torch.zeros(1, 5, hidden_size),
        #         torch.zeros(1, 5, hidden_size))


    def forward(self, _x):
        # 20，5，1->20,5,16 输入神经元1，隐藏层神经元16
        x,_ = self.lstm(_x)  # _x is input, size (seq_len, batch, input_size)
        # x, self.hidden = self.lstm(_x, self.hidden)  # _x is input, size (seq_len, batch, input_size)
        s, b, h = x.shape  # x is output, size (seq_len, batch, hidden_size)

        # 20，5，16->100,16 类似reshape
        x = x.view(s * b, h)
        # 100,16->100,1
