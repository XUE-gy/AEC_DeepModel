...
def parse_ags1():
    ...
    parser.add_argument("--model_name", type=str, default='/740.pth', help="是否加载模型继续训练  None")
    parser.add_argument("--batch-size", type=int, default=16, help="")
    parser.add_argument("--epochs", type=int, default=1000, help='20')
    parser.add_argument('--lr', type=float, default=1e-5, help='学习率 (default: 0.01,3e-4)')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (train_X, train_mask, train_nearend_mic_magnitude, train_nearend_magnitude) in enumerate(
            ...
            train_lsd = pytorch_LSD(train_nearend_magnitude, pred_near_spectrum).to(device)
            train_loss = criterion(train_nearend_magnitude, pred_near_spectrum).to(device)
        ...
        with torch.no_grad():
            for val_batch_idx, (val_X, val_mask, val_nearend_mic_magnitude, val_nearend_magnitude) in enumerate(
                ...
                # 前向传播
                val_pred_mask = model(val_X).to(device)

                # 近端语音信号频谱 = mask * 麦克风信号频谱 [batch_size, 161, 999]
                val_pred_near_spectrum = (val_pred_mask * val_nearend_mic_magnitude).to(device)
                val_lsd = pytorch_LSD(val_nearend_magnitude, val_pred_near_spectrum).to(device)

                val_loss = criterion(val_nearend_magnitude, val_pred_near_spectrum).to(device)
