...
class audio_generator():
    ...
    def create_generator(self):
        ...
        for file in self.file_names:
            ...
            # check if the sampling rates are matching the specifications,检查采样率是否符合规格
            ...
            # count the number of samples in one file,计算一个文件中的样本数
            num_samples = int(np.fix(noisy.shape[0]/self.len_of_samples))
            # iterate over the number of samples,迭代样本数
            for idx in range(num_samples):
                # cut the audio files in chunks,将音频文件切成块
                in_dat = noisy[int(idx*self.len_of_samples):int((idx+1)*
                                                        self.len_of_samples)]
                tar_dat = speech[int(idx*self.len_of_samples):int((idx+1)*
                                                        self.len_of_samples)]
                # yield the chunks as float32 data, 将块生成为 float32 数据
...
class DTLN_model():
    ...
    def __init__(self):
        ...
        # defining default cost function,定义默认成本函数
        self.cost_function = self.snr_cost
        # empty property for the model,模型的空属性
        self.model = []
        # defining default parameters,定义默认参数
        self.fs = 16000
        self.batchsize = 32
        self.len_samples = 15
    ...
    def train_model(self, runName, path_to_train_mix, path_to_train_speech, \
        ...
        # create save path if not existent,如果不存在则创建保存路径
        savePath = './models_'+ runName+'/' 
        if not os.path.exists(savePath):
            os.makedirs(savePath)
        # create log file writer,创建日志文件写入器
        csv_logger = CSVLogger(savePath+ 'training_' +runName+ '.log')
        # create callback for the adaptive learning rate,为自适应学习率创建回调
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5,
                              patience=3, min_lr=10**(-10), cooldown=1)
        # create callback for early stopping,创建提前停止回调
        early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, 
            patience=10, verbose=0, mode='auto', baseline=None)
        # create model check pointer to save the best model,创建模型检查指针以保存最佳模型
        ...
        # calculate length of audio chunks in samples,计算样本中音频块的长度
        len_in_samples = int(np.fix(self.fs * self.len_samples / 
                                    self.block_shift)*self.block_shift)
        # create data generator for training data,为训练数据创建数据生成器
        ...
        # calculate number of training steps in one epoch, 计算一个 epoch 的训练步数
        steps_train = generator_input.total_samples//self.batchsize
        # create data generator for validation data,为验证数据创建数据生成器
        ...
        # calculate number of validation steps,计算验证步骤的数量
        steps_val = generator_val.total_samples//self.batchsize
        # start the training of the model,开始模型的训练
        self.model.fit(
            x=dataset, 
            batch_size=None,
